...
public class AttachementBOImpl implements AttachementBO {
	...
	@Transactional(propagation = Propagation.REQUIRED, readOnly = false, rollbackFor = Exception.class)
	@Override
	public List<Long> addAttachementBatch(List<AttachementDto> attachementDtoList,
		...
		if (CollectionUtils.isNotEmpty(attachementDtoList)) {
			for (AttachementDto attachementDto:attachementDtoList) {
				...
				resList.add(addAttachement(attachementDto));
			}
		}
		return resList;
	}
	...
	@Transactional(propagation = Propagation.REQUIRED, readOnly = false, rollbackFor = Exception.class)
	@Override
	public List<Long> modifyAttachementBatch(List<AttachementDto> attachementDtoList,
			Long objectId, AttachementBizTypeEnum bizTypeEnum,OperatorDto operatorDto)
			throws AugeServiceException {
		ValidateUtils.notNull(objectId);
		ValidateUtils.notNull(bizTypeEnum);
		...
		return addAttachementBatch(attachementDtoList, objectId, bizTypeEnum,operatorDto);
	}
