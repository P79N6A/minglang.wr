...
public class PartnerAdzoneServiceImpl implements PartnerAdzoneService {
	...
	private static final String ADZONE_CACHE_PRE = "ADZONE_";
	...
	public String createAdzoneWithoutStationId(Long taobaoUserId) {
		...
		if (!result.isSuccess()) {
			logger.error(PARTNER_ADZONE_ERROR + "create adzone without station_id error: {}, {}",
					JSON.toJSONString(variables), result.toString());
	...
	public PartnerAdzoneInfoDto getPartnerAdzoneInfoByPid(String pid) {
		...
		d = partnerAdzoneBO.getPartnerAdzoneInfoByPid(pid);

		if (d != null) {
			tairCache.put(cacheKey, d, 3600);
		}
		return d;
	}
