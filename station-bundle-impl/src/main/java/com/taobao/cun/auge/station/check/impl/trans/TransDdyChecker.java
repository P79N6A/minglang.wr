...
public class TransDdyChecker  implements StationTransChecker {
	...
	public void check(PartnerInstanceTransDto t) {
		XzCheckResult xzCheckResult = ddyLbsQueryService.xzCheck(PositionUtil.converDown(t.getStationDto().getAddress().getLng()), PositionUtil.converDown(t.getStationDto().getAddress().getLat()), getIndustry(t.getType().getToBizType().getCode()));
		if(xzCheckResult != null && xzCheckResult.getXzStations() != null && xzCheckResult.getXzStations().size()>0){
			 throw new AugeBusinessException(AugeErrorCodes.PARTNER_INSTANCE_BUSINESS_CHECK_ERROR_CODE, "1公里范围已经有村淘同类网点，请重新选择合适的门店地址。");
		}
	}
	...
	private String getIndustry(String bizType){
		if(StationBizTypeEnum.YOUPIN.getCode().equals(bizType)) {
			return "FMCG";
		}else if (StationBizTypeEnum.YOUPIN_ELEC.getCode().equals(bizType)|| StationBizTypeEnum.TPS_ELEC.getCode().equals(bizType)){
			return "ELEC";
		}
		return null;
	}
