...
public class GeneralTaskSubmitServiceImpl implements GeneralTaskSubmitService {
    ...
    public void submitSettlingSysProcessTasks(PartnerInstanceDto instance, String operator) {
        ...
        GeneralTaskDto unionAdzoneGeneralTaskDto = buildCreateUnionAdzoneTask(instance, operator);
        unionAdzoneGeneralTaskDto.setBusinessStepNo(4L);
        taskDtos.add(unionAdzoneGeneralTaskDto);

        // TODO 旺旺打标 begin
        GeneralTaskDto wangwangGeneralTaskDto = buildAddWangwangTagTask(instance, operator);
        wangwangGeneralTaskDto.setBusinessStepNo(5L);
    ...
    public GeneralTaskDto buildCreateUnionAdzoneTask(PartnerInstanceDto instance, String operator) {
        ...
        taskDto.setBeanName("partnerAdzoneService");
        taskDto.setMethodName("createAdzone");

        taskDto.setBusinessType(TaskBusinessTypeEnum.SETTLING_SYS_PROCESS.getCode());
        taskDto.setBusinessStepDesc("createAdzone");
        taskDto.setOperator(operator);
        taskDto.setPriority(TaskPriority.HIGH);

        taskDto.setParameterType(java.lang.Long.class.getName());
        taskDto.setParameter(JSON.toJSONString(instance.getTaobaoUserId()));
        return taskDto;
    }
