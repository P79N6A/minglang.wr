...
public class StoreWriteBOImpl implements StoreWriteBO {
	...
	private String createInventoryStore(StoreCreateDto storeCreateDto, Long userId, String areaId)
	...
	public Boolean createSupplyStore(Long stationId) {
		 ...
		 * (cainiaoStationId != null) { LinkedHashMap<String, String> features =
		 * new LinkedHashMap<String, String>(); features.put("goodsSupply",
	...
	public Boolean initSampleWarehouse(Long stationId) {
		try {
			...
			return initCtMdJxcWarehouse(BooleanStatusEnum.NO, storeDto.getSellerShareStoreId() + "",
					partnerInstance.getSellerId());
		} catch (Exception e) {
	...
	public void syncStore(Long stationId) {
		try {
			...
			logger.error("syncStore error["+stationId+"]",e);
		}
