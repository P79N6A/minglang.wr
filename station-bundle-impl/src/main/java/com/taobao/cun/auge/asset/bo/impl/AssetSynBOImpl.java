...
public class AssetSynBOImpl implements AssetSynBO {
	...
	private static final Logger logger = LoggerFactory.getLogger(AssetSynBO.class);
	...
	private void addDistributeRolloutInfo(Asset a, CuntaoAsset ca) {
		...
		PartnerStationRel psl = partnerInstanceBO.findPartnerInstanceById(ca.getPartnerInstanceId());
	...
	private Asset bulidStationUseAsset(CuntaoAsset cuntaoAsset) {
		...
		if (cuntaoAsset.getPartnerInstanceId() != null) {
			PartnerStationRel psl = partnerInstanceBO.findPartnerInstanceById(cuntaoAsset.getPartnerInstanceId());
	...
	// List<CuntaoUser> userLists = cuntaoUserService.getCuntaoUsers(orgId,
	// UserRole.COUNTY_LEADER);
		...
		if (CollectionUtils.isNotEmpty(assetList)) {
			...
			for (Asset a : assetList) {
				try {
					...
					if (a.getStatus().equals(AssetStatusEnum.TRANSFER.getCode())||a.getStatus().equals(AssetStatusEnum.SCRAP.getCode())) {
		...
		if (resDto.isSuccess()) {
			...
			if (a.getStatus().equals(AssetStatusEnum.SIGN.getCode())) {
				if (!(pDto.getStatus().equals("Stocking")) || !(pDto.getStatusDetail().equals("Available"))) {
					logger.error("sign.check.asset.error,aliNo=" + a.getAliNo() + ":status=" + pDto.getStatus()
							+ ":statusDetail=" + pDto.getStatusDetail());
				}
			}

			if (a.getStatus().equals(AssetStatusEnum.SCRAP.getCode())) {
				if (!(pDto.getStatus().equals("Scrapped"))) {
					logger.error("scrap.check.asset.error,aliNo=" + a.getAliNo() + ":status=" + pDto.getStatus()
							+ ":statusDetail=" + pDto.getStatusDetail());
				}
			}
			...
			if (a.getStatus().equals(AssetStatusEnum.USE.getCode())
				...
				if (!(pDto.getStatus().equals("Using"))) {
					logger.error("use.check.asset.error,aliNo=" + a.getAliNo() + ":orgstatus=" + a.getStatus()
							+ ":status=" + pDto.getStatus() + ":statusDetail=" + pDto.getStatusDetail());
				}
			}
		} else {
			logger.error("getAssetInfo error,aliNo=" + a.getAliNo() + ":orgstatus=" + a.getStatus(),
					resDto.getErrorMsg());
		}
	}
	...
	public boolean scrapAssetByOrg(List<String> aliNoList) {
		AssetExample cuntaoAssetExample = new AssetExample();
		List<Asset> assetList = new ArrayList<Asset>();
		cuntaoAssetExample.createCriteria().andIsDeletedEqualTo("n").andAliNoIn(aliNoList);
		assetList = assetMapper.selectByExample(cuntaoAssetExample);
	...
	public void checkAssetToAmpForBcp(Long assetId) {
		...
		if (resDto.isSuccess()) {
			...
			if (a.getStatus().equals(AssetStatusEnum.SIGN.getCode())) {
				if (!(pDto.getStatus().equals("Stocking")) || !(pDto.getStatusDetail().equals("Available"))) {
					logger.error("checkAssetToAmpForBcp.sign.check.error,aliNo=" + a.getAliNo() + ":status="
							+ pDto.getStatus() + ":statusDetail=" + pDto.getStatusDetail());
					throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
							"【" + a.getAliNo() + "】是待县签收资产，集团状态必须是库存-可用。");
				}
			}

			/*
			 * if (a.getStatus().equals(AssetStatusEnum.SCRAP.getCode())) { if
			 * (!(pDto.getStatus().equals("Scrapped"))) {
			 * logger.error("scrap.check.asset.error,aliNo="+a.getAliNo()+
			 * ":status="+pDto.getStatus()+":statusDetail="+pDto.getStatusDetail
			 * ()); } }
			 */
			...
			if (a.getStatus().equals(AssetStatusEnum.USE.getCode())
				...
				if (pDto.getStatus().equals("Using")) {
					if (!(a.getOwnerWorkno().equals(pDto.getOwner()))) {
						logger.error("checkAssetToAmpForBcp.use.check.owner.error,aliNo=" + a.getAliNo() + ":orgOwner="
								+ a.getOwnerWorkno() + ":ampOwner=" + pDto.getOwner());
						throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "【" + a.getAliNo()
								+ "】责任人不一致，org责任人是" + a.getOwnerWorkno() + ":集团资产责任人=" + pDto.getOwner());
					}
				}
			...
			logger.error(
					"checkAssetToAmpForBcp.getAssetInfo.error,aliNo=" + a.getAliNo() + ":orgstatus=" + a.getStatus(),
					resDto.getErrorMsg());
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
					"【" + a.getAliNo() + "】获取集团数据失败。");
		}
	...
	public Boolean disAsset(String aliNo, Long userAreaId, String userName, String userId) {
		...
		AssetExample cuntaoAssetExample = new AssetExample();
		List<Asset> assetList = new ArrayList<Asset>();
		cuntaoAssetExample.createCriteria().andIsDeletedEqualTo("n").andAliNoEqualTo(aliNo);
		assetList = assetMapper.selectByExample(cuntaoAssetExample);
		if (CollectionUtils.isNotEmpty(assetList)) {
			for (Asset a : assetList) {
				try {
					...
					if (a.getStatus().equals(AssetStatusEnum.TRANSFER.getCode())
						...
						if (detail != null) {
							...
							if (incomeId != null) {
								// 更新出入库单状态
								if (assetRolloutIncomeDetailBO.isAllSignByIncomeId(incomeId)) {
									assetIncomeBO.updateStatus(incomeId, AssetIncomeStatusEnum.DONE, "fenfa");
								} else {
									assetIncomeBO.updateStatus(incomeId, AssetIncomeStatusEnum.DOING, "fenfa");
								}
							}
							if (rolloutId != null) {
								if (assetRolloutIncomeDetailBO.isAllSignByRolloutId(rolloutId)) {
									...
									assetRolloutBO.updateStatus(rolloutId, AssetRolloutStatusEnum.ROLLOUT_ING, "fenfa");
								}
							}
					...
					logger.error("scrap asset error,asset=" + JSONObject.toJSONString(a), e);
				}
			}
		}
