...
public class AssetBOImpl implements AssetBO {
	...
	private AssetChangeEvent buildAssetChangeEvent(Long assetId, String type, String operator, OperatorTypeEnum opType,
			String desc) {
	...
	public List<CategoryAssetListDto> getCategoryAssetList(AssetOperatorDto operatorDto) {
		...
		for (Entry<String, List<Asset>> entry : listMap.entrySet()) {
			...
			listDto.setPutAway(
					String.valueOf(list.stream().filter(i -> AssetStatusEnum.DISTRIBUTE.getCode().equals(i.getStatus())
							|| AssetStatusEnum.TRANSFER.getCode().equals(i.getStatus())).count()));
	...
	public CategoryAssetDetailDto getCategoryAssetDetail(AssetDetailQueryCondition condition) {
		...
		assetExample.createCriteria().andIsDeletedEqualTo("n").andOwnerWorknoEqualTo(condition.getOperator())
				...
				.andOwnerOrgIdEqualTo(condition.getOperatorOrgId());
		// 组织头部
		...
		assetDetailDto.setPutAway(
				String.valueOf(preAssets.stream().filter(i -> AssetStatusEnum.DISTRIBUTE.getCode().equals(i.getStatus())
						|| AssetStatusEnum.TRANSFER.getCode().equals(i.getStatus())).count()));
	...
	public AreaAssetDetailDto getAreaAssetDetail(AssetDetailQueryCondition condition) {
		...
		AssetExample conditionExample = new AssetExample();
		AssetExample.Criteria criteria = conditionExample.createCriteria();
		criteria.andIsDeletedEqualTo("n").andUseAreaTypeEqualTo(condition.getUseAreaType())
				.andOwnerWorknoEqualTo(condition.getOperator()).andUseAreaIdEqualTo(condition.getUseAreaId())
				.andOwnerOrgIdEqualTo(condition.getOperatorOrgId());
		// 组织尾巴
		if (StringUtils.isNotEmpty(condition.getStatus())) {
			if ("Y".equals(condition.getStatus())) {
	...
	public AssetDetailDto signAssetByCounty(AssetDto signDto) {
		...
		assetExample.createCriteria().andIsDeletedEqualTo("n").andAliNoEqualTo(signDto.getAliNo())
				.andStatusIn(AssetStatusEnum.getCanCountySignStatusList());
	...
	private void obtainItAsset(AssetDto signDto) {
		...
		if (!result.isSuccess() || !result.getResult()) {
			...
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "集团资产出库失败,请联系管理员！");
		}
	}
	...
	public void transferItAsset(AssetDto signDto) {
		...
		transDto.setWorkId(LoginIdConverter.convertTo6Bit(signDto.getOperator()));
		transDto.setUser(LoginIdConverter.convertTo6Bit(signDto.getOperator()));
		AssetApiResultDO<Boolean> result = cuntaoApiService.transferOwner(Collections.singletonList(transDto));
		if (!result.isSuccess()) {
			logger.error("{bizType}, transferItAsset error,{errorMsg} ", "assetError", result.getErrorMsg());
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "集团资产转移失败,请联系管理员！");
		}
	}
	...
	public Boolean signAssetByStation(AssetDto signDto) {
		...
		AssetExample assetExample = new AssetExample();
		assetExample.createCriteria().andIsDeletedEqualTo("n").andAliNoEqualTo(signDto.getAliNo())
				.andStatusEqualTo(AssetStatusEnum.DISTRIBUTE.getCode());
		Asset asset = ResultUtils.selectOne(assetMapper.selectByExample(assetExample));
		if (asset == null) {
	...
	public AssetDetailDto recycleAsset(AssetDto signDto) {
		...
		if (!asset.getOwnerWorkno().equals(signDto.getOperator())
				|| !asset.getOwnerOrgId().equals(signDto.getOperatorOrgId())) {
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
	...
	public List<Asset> transferAssetOtherCounty(AssetTransferDto transferDto) {
		...
		AssetExample.Criteria criteria = assetExample.createCriteria();
		criteria.andIsDeletedEqualTo("n").andOwnerWorknoEqualTo(transferDto.getOperator())
				.andIdIn(transferDto.getTransferAssetIdList()).andOwnerOrgIdEqualTo(transferDto.getOperatorOrgId());
		List<Asset> assetList = assetMapper.selectByExample(assetExample);
		if (!assetList.stream().allMatch(asset -> AssetStatusEnum.USE.getCode().equals(asset.getStatus()))) {
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "您转移的资产中包含待对方入库的资产");
		}
		if (!assetList.stream()
				.allMatch(asset -> AssetUseAreaTypeEnum.COUNTY.getCode().equals(asset.getUseAreaType()))) {
	...
	private List<AssetCategoryCountDto> buildAssetCountDtoList(List<Asset> list) {
		...
		for (Entry<String, List<Asset>> countEntry : countListMap.entrySet()) {
			...
			assetCountDto
					.setPutAway(
							String.valueOf(
									countEntry.getValue().stream()
											.filter(i -> AssetStatusEnum.DISTRIBUTE.getCode().equals(i.getStatus())
													|| AssetStatusEnum.TRANSFER.getCode().equals(i.getStatus()))
											.count()));
	...
	public AssetDetailDto getScrapDetailById(Long id, AssetOperatorDto assetOperatorDto) {
		...
		AssetApiResultDO<AssetLostQueryResult> queryResult = cuntaoApiService.assetLostQuery(detailDto.getAliNo(),
				GROUP_CODE);
		if (!queryResult.isSuccess()) {
			logger.error("{bizType}, getScrapDetailById error,{errorMsg} ", "assetError", queryResult.getErrorMsg());
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "查询资产净值失败,请联系管理员！");
		}
	...
	private void scrapingItAsset(Asset asset, AssetScrapDto scrapDto) {
		...
		if (!result.isSuccess()) {
			logger.error("{bizType},{parameter} scraping it asset fail " + result.getErrorMsg(), "assetError",
					JSON.toJSONString(requestDto));
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "集团资产发起赔付失败,请联系管理员！");
		}
	}
	...
	private void scrapItAsset(Asset asset, AssetScrapDto scrapDto) {
		...
		if (!result.isSuccess()) {
			logger.error("{bizType},{parameter} scrap it asset fail " + result.getErrorMsg(), "assetError",
					JSON.toJSONString(requestDto));
			// throw new
			// AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
			// "集团资产赔付失败,请联系管理员！");
		}
	}
	...
	private void transferBail(AssetScrapDto scrapDto, String userId) {
		...
		if (!resultModel.isSuccess()) {
			...
			throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "资产赔付保证金转移失败,请联系管理员！");
		}
	}
	...
	public List<Asset> distributeAsset(AssetDistributeDto distributeDto) {
		 ...
		 * 
		 * if (CollectionUtils.isEmpty(distributeDto.getAssetIdList())) { throw
		 * new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "待分发资产不能为空!"); } Station s =
		 * stationBO.getStationById(distributeDto.getStationId()); if (s ==null
		 * || !(canDistributeList().contains(s.getStatus()))) { throw new
		 * AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "当前服务站状态不能分发资产!"); }
		 * 
		 * AssetExample assetExample = new AssetExample(); AssetExample.Criteria
		 * criteria = assetExample.createCriteria();
		 * criteria.andIsDeletedEqualTo("n").andOwnerWorknoEqualTo(distributeDto
		 * .getOperator()).andIdIn( distributeDto.getAssetIdList()); List<Asset>
		 * assetList = assetMapper.selectByExample(assetExample); if
		 * (!assetList.stream().allMatch(asset ->
		 * AssetStatusEnum.USE.getCode().equals(asset.getStatus()))) { throw new
		 * AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "您分发的资产中包含待对方入库的资产!"); } if (!assetList.stream().allMatch( asset ->
		 * AssetUseAreaTypeEnum.COUNTY.getCode().equals(asset.getUseAreaType()))
		 * ) { throw new
		 * AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "您分发的资产中包含已下发至村点的资产!"); } Asset asset = new Asset();
		 * DomainUtils.beforeUpdate(asset, distributeDto.getOperator());
		 * asset.setStatus(AssetStatusEnum.DISTRIBUTE.getCode());
		 * assetMapper.updateByExampleSelective(asset, assetExample); return
		 * assetList;
		 */
	}
	...
	public Page<Asset> getCheckedAsset(Integer pageNum, Integer pageSize) {
		...
		assetExample.createCriteria().andIsDeletedEqualTo("n")
				.andCheckStatusEqualTo(AssetCheckStatusEnum.CHECKED.getCode())
				.andStatusEqualTo(AssetStatusEnum.SCRAP.getCode());
		PageHelper.startPage(pageNum, pageSize);
	...
	private AssetExample bulidStationAssetParam(Long stationId, Long taobaoUserId) {
		...
		assetExample.createCriteria().andIsDeletedEqualTo("n").andUseAreaIdEqualTo(stationId)
				...
				.andUserIdEqualTo(String.valueOf(taobaoUserId)).andStatusNotIn(sList);
		return assetExample;
	}
	...
	public List<AssetDetailDto> getDistributeAssetListByStation(Long stationId, Long taobaoUserId) {
		...
		for (AssetRollout ar : arList) {
			...
			List<AssetRolloutIncomeDetail> waitSignList = deList.stream()
					.filter(i -> AssetRolloutIncomeDetailStatusEnum.WAIT_SIGN.getCode().equals(i.getStatus()))
					.collect(Collectors.toList());
			assetIdList.addAll(
	...
	public Map<String, String> getStationAssetState(Long stationId) {
		...
		// result.put("canBuy",
		// String.valueOf(diamondConfiguredProperties.getCanBuyStationList().contains(stationId)));
	...
	public Map<String, String> buyAsset(CuntaoAssetDto assetDto) {
		...
		if ("true".equals(result.get("success"))) {
			...
			if (resultModel.isSuccess() && resultModel.getResult()) {
				scrapBuyAsset(assetList.stream().map(Asset::getId).collect(Collectors.toList()),
						assetDto.getOperator());
	...
	private List<Asset> checkAssetState(CuntaoAssetDto assetDto, Map<String, String> result) {
		...
		// if
		// (!diamondConfiguredProperties.getCanBuyStationList().contains(stationId))
		// {
		// result.put("message", "该村点不允许提交资产采购意向,请联系资产管理员!");
		// result.put("success", "false");
		// return null;
	...
	private boolean validateStationAssetNum(List<Asset> assetList) {
		...
		List<String> categoryList = assetList.stream().map(Asset::getCategory).collect(Collectors.toList());
	...
	private void saveBuyRecord(CuntaoAssetDto assetDto, List<Asset> assetList) {
		...
		record.setRemarks(assetList.stream().map(Asset::getId).map(String::valueOf).collect(Collectors.joining(",")));
	...
	public AssetDetailDto judgeDistribute(AssetDto assetDto) {
		...
		/*
		 * Objects.requireNonNull(assetDto.getAliNo(), "编号不能为空");
		 * Objects.requireNonNull(assetDto.getOperator(), "操作人不能为空"); Asset
		 * asset = getAssetByAliNo(assetDto.getAliNo()); if (asset == null) {
		 * throw new
		 * AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "录入失败" + AssetBO.NO_EXIT_ASSET); } if
		 * (!asset.getOwnerWorkno().equals(assetDto.getOperator())) { throw new
		 * AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "录入失败" + AssetBO.NOT_OPERATOR + getPromptInfo(asset)); } if
		 * (!AssetStatusEnum.USE.getCode().equals(asset.getStatus())) { throw
		 * new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "录入失败,该资产正处于分发、转移中！" + getPromptInfo(asset)); } if
		 * (!asset.getOwnerOrgId().equals(assetDto.getOperatorOrgId())) { throw
		 * new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE,
		 * "录入失败,该资产不属于当前组织！" + getPromptInfo(asset)); } return
		 * buildAssetDetail(asset);
		 */
	}
	...
	public Boolean signAllAssetByCounty(AssetSignDto signDto) {
		...
		List<Long> idList = assetRolloutIncomeDetailBO.queryWaitSignListByIncomeId(signDto.getIncomeId()).stream()
				.map(AssetRolloutIncomeDetail::getAssetId).collect(Collectors.toList());
		if (CollectionUtils.isNotEmpty(idList)) {
			...
			if (!assetList.stream()
					.allMatch(asset -> AssetUseAreaTypeEnum.STATION.getCode().equals(asset.getUseAreaType()))) {
				throw new AugeBusinessException(AugeErrorCodes.ASSET_BUSINESS_ERROR_CODE, "签收失败,该资产不处于村点使用下!");
			}
			assetMapper.updateByExampleSelective(updateAsset, example);
			// 批量调集团接口转移资产责任人
			assetList.stream().map(asset -> {
	...
	private List<Asset> getStationThreeAsset(Long stationId, Long taobaoUserId) {
		...
		assetExample.createCriteria().andIsDeletedEqualTo("n").andUseAreaIdEqualTo(stationId)
				.andUseAreaTypeEqualTo(AssetUseAreaTypeEnum.STATION.getCode())
				.andStatusEqualTo(AssetStatusEnum.USE.getCode()).andCategoryIn(cateList)
				.andUserIdEqualTo(String.valueOf(taobaoUserId));
		return assetMapper.selectByExample(assetExample);
	}
	...
	public Map<String, String> validateThreeAsset(ValidateThreeAssetDto vaDto) {
		...
		for (Asset a : rs) {
			if ("TV".equals(a.getCategory())) {
				if (StringUtils.isEmpty(vaDto.getTvPostfix())
						|| !vaDto.getTvPostfix().equals(getSubAliNo(a.getAliNo()))) {
					result.put("message", validateThreeAssetTips("电视机", vaDto.getTvPostfix()));
					result.put("success", "false");
					return result;
				}
			}
			if ("DISPLAY".equals(a.getCategory())) {
				if (StringUtils.isEmpty(vaDto.getDisplayPostfix())
						|| !vaDto.getDisplayPostfix().equals(getSubAliNo(a.getAliNo()))) {
					result.put("message", validateThreeAssetTips("显示器", vaDto.getDisplayPostfix()));
					result.put("success", "false");
					return result;
				}
			}
			if ("MAIN".equals(a.getCategory())) {
				if (StringUtils.isEmpty(vaDto.getMainPostfix())
						|| !vaDto.getMainPostfix().equals(getSubAliNo(a.getAliNo()))) {
					result.put("message", validateThreeAssetTips("主机", vaDto.getMainPostfix()));
					result.put("success", "false");
					return result;
				}
			}
	...
	private String validateThreeAssetTips(String categoryName, String pos) {
		return "对不起,该村点不符合自购资格，您提供的" + categoryName + "资产编号[" + pos
				+ "]和您当初签收的不一致，请检查后重新输入；若仍提示错误，请将设备上的准确编码提供给您对应的县运营小二处理。";
	}
